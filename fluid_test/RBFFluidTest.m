function RBFFluidTest(N, P, rho, mu)
    alpha = P(:, 1);
    beta = P(:, 2);
    ...
    Force = ForceCalculator(alpha, beta, x, y, z, 1, 1e2, 1e-12);
    pts = size(P, 1);
    sa = 4 * pi * R^2 / pts;
    ...
    for m = 1:30
        for n = 1:sps
            ...
            Fex = BetterTransfer('l2e', N, [x y z], [0 0.5 0.5] / N, Fl(:, 1), sa);
            Fey = BetterTransfer('l2e', N, [x y z], [0.5 0 0.5] / N, Fl(:, 2), sa);
            Fez = BetterTransfer('l2e', N, [x y z], [0.5 0.5 0] / N, Fl(:, 3), sa);
            Fe = [Fex Fey Fez];
            ...
            [Fls, Flb] = Force(xi, yi, zi);
            Fl = Fls + Flb;

            Fex = BetterTransfer('l2e', N, [xi yi zi], [0 0.5 0.5] / N, Fl(:, 1), sa);
            Fey = BetterTransfer('l2e', N, [xi yi zi], [0.5 0 0.5] / N, Fl(:, 2), sa);
            Fez = BetterTransfer('l2e', N, [xi yi zi], [0.5 0.5 0] / N, Fl(:, 3), sa);
            Fe = [Fex Fey Fez];
